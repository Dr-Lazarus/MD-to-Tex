...
void printHelp(void) {
  ...
  printf(
      ...
      "extension.\n");
  printf(
      "  -d Switches on debugging capability and prints line by line data as "
      "well as the Abstract Syntax Tree\n");
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--h") == 0) {
      ...
      printf("Debugging mode on, will print AST");
      debugging = 1;
    }
  }
  ...
  md_node *document = parse_source(inputFileName, debugging);
  if (debugging) {
