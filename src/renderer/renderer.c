...
int parse_graph_line(char* line, char* fromNode, char* toNode) {
    ...
    if (arrowPtr != NULL) {
        *arrowPtr = '\0'; 
        ...
        while (isspace((unsigned char)*line)) line++; // Skips leading spaces in fromNode
        while (isspace((unsigned char)*(arrowPtr))) arrowPtr++; // Skips leading spaces in toNode
...
void addNode(char* node_list, char node) {
    ...
    for (int i = 0; i < len; i++) {
        if (node_list[i] == node) return; // Node already exists
    }
    node_list[len] = node; // Add new node
    node_list[len + 1] = '\0';
}

void parse_mermaid_code(const char* mermaid_code, char* node_list, Edge* edge_list, int* edge_count) {
    ...
    *edge_count = 0;
    ...
    while (*code_cursor != '\0') {
        ...
        while (isspace((unsigned char)*code_cursor)) code_cursor++;
...
void calculate_layout(char* node_list, Edge* edge_list, int edge_count, Position* positions) {
    ...
    for (int i = 0; i < edge_count; i++) {
        ...
        for (int j = 0; layoutNodes[j].id != '\0'; j++) {
            if (layoutNodes[j].id == fromId) fromIndex = j;
            if (layoutNodes[j].id == toId) toIndex = j;
        }
        ...
        if (fromIndex != -1 && toIndex != -1) {
            ...
            layoutNodes[toIndex].level = layoutNodes[fromIndex].level + 1; // Next level
...
void generate_latex(char* node_list, Position* positions, Edge* edge_list, int edge_count, FILE* outputFile) {
    fprintf(outputFile, "\\begin{tikzpicture}[>=stealth]\n");

    // Node
    for (int i = 0; node_list[i] != '\0'; i++) {
        fprintf(outputFile, "\\node (%c) at (%d, %d) {%c};\n",
                node_list[i], positions[i].x, positions[i].y, node_list[i]);
    }
